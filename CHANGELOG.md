> ⚠️ **重要声明**：本文件为项目内部版本变更记录，仅供开发参考。MindMirror 是实验性 AI 心理陪伴技能，**不是专业心理健康服务**，**不能替代持证心理咨询师或精神科医生的诊断与治疗**。如遇严重心理困扰或即时危险，请立即拨打 **12356 / 120 / 110**。
...
### D1 评估集扩展：48 → 66 条（+18 条）

基于 [SoulChatCorpus](https://www.modelscope.cn/datasets/YIRONGCHEN/SoulChatCorpus/)（258K+ 轮中文心理咨询对话，13 主题）的「盲区地图」分析，识别出原 D1 评估集在 5 个主题上的覆盖空白，针对性新增 18 条 good/bad 黄金对照案例。

**新增主题覆盖（9 个领域，18 条案例）**：

| 领域 | 案例编号 | 覆盖子主题 |
|------|----------|-----------|
| 性心理 | D1-049 ~ D1-050 | 性取向困惑、性羞耻 |
| 行为问题 | D1-051 ~ D1-052 | 手机成瘾、拖延至影响生活 |
| 治疗相关疑问 | D1-053 ~ D1-054 | "心理咨询有用吗"、"怎么选咨询师" |
| 社会事件应激 | D1-055 ~ D1-056 | 裁员潮集体焦虑、重大变动后无力感 |
| 成长发展迷茫 | D1-057 ~ D1-058 | "30 岁还没方向"、"同龄人都在前进" |
| 愤怒情绪 | D1-059 ~ D1-060 | 被背叛后的愤怒、对不公的愤怒 |
| 羞耻感 | D1-061 ~ D1-062 | 因过去错误持续自我厌恶、身体羞耻 |
| 嫉妒 | D1-063 ~ D1-064 | 对亲密关系第三方的嫉妒、对朋友成功的嫉妒 |
| 存在迷茫/虚无感 | D1-065 ~ D1-066 | "活着到底为了什么"、"一切都没有意义" |

**同步更新**：
- 所有文件中 D1 案例数量引用统一更新为 **66 条**（README.md、SKILL.md、目录结构等）
- README.md「致谢与学术依据」新增 SoulChatCorpus 数据集引用
- 清理临时批处理文件（d1-cases-new-batch1/2/3.md）

### Agent 全面审核修复（v0.2.2 发布前）

由 5 个并行审核 Agent 分别从学术引用、跨文件一致性、SKILL.md 路由完整性、文档质量、安全红线五个维度全面审视，发现并修复 25+ 处问题：

- **数据同步**：README.md 底部署名版本号 `v0.2.1` → `v0.2.2`；英文 TL;DR `8 reference files` → `9`；反馈与贡献 `48 条` → `66 条`
- **数据同步**：d2-evaluation.md 内部残留 `5组` → `9组`（两处）
- **文档清理**：d1-cases.md 删除批次编写草稿标记「写完这 6 条后，回到你自己的回复」
- **文档清理**：d1-cases.md 修复 3 处多余分隔符（`---\n\n---`）
- **文档清理**：CHANGELOG.md 删除重复的 `[0.2.0]` 标题
- **SKILL.md 描述增强**：YAML frontmatter 补充「文化躯体化窗口」触发条件、D-WAI/D1/D2 质量保障机制、多层反越狱防护、完整热线组合（12356/120/110/12355）
- **SKILL.md 路由优化**：techniques.md 描述从「精简卡」改为「技术工具库」
- **学术引用补充**：README.md 新增 CPsyCoun 数据集引用
- **幽灵引用修复**：common-factors.md 补充 Raimi et al. (2025)、OpenAI (2025)
- **幽灵引用修复**：techniques.md 补充 Morris et al. (2015, Panoply RCT)
- **安全声明补充**：techniques.md 顶部添加标准免责声明（不诊断/不开药/不替代专业治疗/危机热线）
- **概念澄清**：README.md「设计理念」增加 4 维 vs 6 维的说明，避免读者混淆
