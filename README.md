...
具体路径请查阅你所用客户端的 Skill 文档。把整个仓库以 `mindmirror/` 为名拷进去即可——识别逻辑是统一的：通过 [SKILL.md](SKILL.md) 顶部的 YAML frontmatter 自动路由。
...
装好后开新会话发一句"今天好累"，回复风格会切到"心镜模式"。也可以按你客户端的方式显式触发（如 `/mindmirror` 或菜单选择）验证。
...
| 校准你自己模型的回复品味 | 用 [d1-cases.md](references/d1-cases.md) 的 15 条对照做 few-shot 或离线评测 |
...
- 共同要素、危机协议、量表题干、技术速查 —— 都是**通用心理学内容**，跨模型可直接复用
